...
func TestSveAndLoadWithMultipleShards(t *testing.T) {
	tmpDir := t.TempDir()
	sourceFile := "test.txt"
	chunkSize := 4096
	hyperplanes := simhash.GenerateHyperplanes(128, 64)

	// Create index and force shard retation
	idx := New(sourceFile, chunkSize, hyperplanes, tmpDir)
	for i := 0; i < MaxShardSize; i++ {
		if err := idx.Add(simhash.SimHash(i), int64(i)); err != nil {
			t.Fatalf("Add failed: %v", err)
		}
	}
	hashSecondShard := simhash.SimHash(MaxShardSize)
	posSecondShard := int64(1000)
	if err := idx.Add(hashSecondShard, posSecondShard); err != nil {
		t.Fatalf("Add failed: %v", err)
	}
	...
	// Verify fiest shard (loaded immeediately)
	posFirstShard, err := loadedIdx.Lookup(simhash.SimHash(0))
	if err != nil || len(posFirstShard) != 1 || posFirstShard[0] != 0 {
		t.Errorf("Expected position 0 for hash 0; got %v, posFirstShard", posFirstShard)
	}

	// Verify second shard (loaded on demand)
	posSecond, err := loadedIdx.Lookup(hashSecondShard)
	if err != nil || len(posSecond) != 1 || posSecond[0] != posSecondShard {
		t.Errorf("Expected postion %d for hash %x, got %v", posSecondShard, hashSecondShard, posSecond)
	}
}

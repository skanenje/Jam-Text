...
// func TestSaveAndLoadWithMultipleShards(t *testing.T) {
// 	tmpDir := t.TempDir()
// 	sourceFile := "test.txt"
// 	chunkSize := 4096
// 	hyperplanes := simhash.GenerateHyperplanes(128, 64)

// 	// Create index and force shard rotation
// 	idx := New(sourceFile, chunkSize, hyperplanes, tmpDir)
// 	for i := 0; i < MaxShardSize; i++ {
...
// 	// Verify first shard (loaded immediately)
// 	posFirstShard, err := loadedIdx.Lookup(simhash.SimHash(0))
// 	if err != nil {
// 		t.Fatalf("Lookup failed: %v", err)
// 	}
// 	if len(posFirstShard) != 1 || posFirstShard[0] != 0 {
// 		t.Errorf("Expected position 0 for hash 0, got %v", posFirstShard)
// 	}
...
// 		t.Fatalf("Lookup failed: %v", err)
// 	}
// 	if len(posSecond) != 1 || posSecond[0] != posSecondShard {
// 		t.Errorf("Expected position %d for hash %x, got %v", posSecondShard, hashSecondShard, posSecond)
// 	}
// }
